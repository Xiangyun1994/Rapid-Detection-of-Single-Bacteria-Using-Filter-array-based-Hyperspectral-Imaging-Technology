...
def img2tensor(imgs, bgr2rgb=True, float32=True):
    ...
    def _totensor(img, bgr2rgb, float32):
        if img.shape[2] == 3 and bgr2rgb and img.shape[2] == 1:
...
def npy2tensor(imgs, float32=True):
    """Numpy array to tensor.
...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    for _tensor in tensor:
        ...
        # step2：将tensor进行一个线性拉伸，拉伸到最大值为1，最小值为0
        ...
        if n_dim == 4:
            ...
            img_np = _tensor.numpy()  # 转为array形式
            img_np = img_np.transpose(1, 2, 0)  # CHW->HWC, RGB->BGR（因为后续要用cv2保存）
...
def tensor2npy(tensor, out_type=np.float32, min_max=(0, 1)):
    ...
    for _tensor in tensor:
        # step1：首先通过squeez()将输入的tensor去掉batch这个维度,变为[c,h,w],
        # 之后转为float和cpu，再将tensor的值限制在[0,1]之间（有时候tensor为负值）
        _tensor = _tensor.squeeze(0).float().detach().cpu().clamp_(*min_max)  # 把数据归一在数据最大最小值之间
        # step2：将tensor进行一个线性拉伸，拉伸到最大值为1，最小值为0
        _tensor = (_tensor - min_max[0]) / (min_max[1] - min_max[0])  # 归一化  to range [0,1]

        n_dim = _tensor.dim()  # n_dim 3
        if n_dim == 4:
            img_np = make_grid(_tensor, nrow=int(math.sqrt(_tensor.size(0))), normalize=False).numpy()
            img_np = img_np.transpose(1, 2, 0)
        elif n_dim == 3:  # 一般情况下会直接跳到这里
...
def imfrombytes(content, flag='color', float32=False):
    ...
    img = cv2.imdecode(img_np, imread_flags[flag])  # 从网络读取图像数据并转换成图片格式：
    if float32:
        img = img.astype(np.float32) / 255.  # 数据类型转化以及归一化
    return img


def npyfrombytes(content):
    """Read an image from npy.
    ...
    Returns:
        ndarray: Loaded image array.
    """
    npy_data = np.load(content)   # 加载npy数据
    npy_data.decode("utf8", "ignore")
    # print(npy_data)
    # npy_data = np.float32(npy_data)
    # npy_data = np.array(npy_data)
    # npy_data = npy_data / np.max(np.hstack(npy_data))  # 归一化
    return npy_data
...
def imwritefromnpy(img, file_path, params=None, auto_mkdir=True):
    """Write image to file.
